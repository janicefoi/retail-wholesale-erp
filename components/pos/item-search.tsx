...
import {
  Search, Trash2, Loader2, ShoppingCart, Package, Plus, X, Tag,
} from "lucide-react";
import {
...
export function ItemSearch() {
  ...
  const [pending, setPending] = useState<SearchedItem | null>(null); // item selected but not yet added
  ...
  const { items: cartItems, addItem, removeItem, updateQuantity, setItemSpecialPrice } = useCartStore();
  ...
  function pickItem(item: SearchedItem) {
    ...
    setPending(item);
    setShowDrop(false);
    setQuery("");       // clear search so cashier can immediately search the next item
    setAddQty(1);
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden">
            ...
            <>
              ...
              <div className="flex-1 min-w-0 flex items-center gap-2 bg-white border border-blue-200 rounded-lg px-2.5 py-1">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-[10px] font-mono text-slate-400">{pending.sku}</p>
                </div>
              ...
              <div className="flex items-center border border-slate-200 rounded-md bg-white overflow-hidden shrink-0">
                <button onClick={() => setAddQty(q => Math.max(1, q - 1))} className="px-2 py-1 text-slate-500 hover:bg-slate-100 font-bold text-sm leading-none">−</button>
                <input
                  ref={qtyRef}
                  type="number"
                  min={1}
                ...
                />
                <button onClick={() => setAddQty(q => q + 1)} className="px-2 py-1 text-slate-500 hover:bg-slate-100 font-bold text-sm leading-none">+</button>
              </div>
              ...
              {/* Add button */}
              <Button size="sm" className="h-8 px-3 gap-1 shrink-0" onClick={commitAdd}>
            ...
            <Table>
              <TableHeader>
                <TableRow className="bg-slate-50 hover:bg-slate-50">
                  ...
                  <TableHead className="w-32 text-right whitespace-nowrap">Unit Price</TableHead>
                  <TableHead className="w-32 text-right whitespace-nowrap">Subtotal</TableHead>
                    ...
                    <TableCell className="text-center">
                      <div className="flex items-center justify-center border border-slate-200 rounded-md overflow-hidden w-fit mx-auto">
                        ...
                        <input
                          ...
                          min={1}
                          value={item.quantity}
                          onChange={e => { const v = parseInt(e.target.value); if (!isNaN(v) && v >= 1) updateQuantity(item.itemId, v); }}
                          className="w-10 text-center text-xs py-0.5 border-x border-slate-200 focus:outline-none"
                        />
                        <button
                          onClick={() => updateQuantity(item.itemId, item.quantity + 1)}
                          className="px-1.5 py-0.5 text-slate-500 hover:bg-slate-100 text-xs font-bold leading-none"
                        >+</button>
                      </div>
                      <button
                        type="button"
      ...
      {/* ══ FOOTER ═══════════════════════════════════════════════════════ */}
      {cartItems.length > 0 && (
        <div className="px-4 py-1.5 border-t border-slate-100 bg-slate-50 text-[11px] text-slate-400">
