...
export function EmployeeDialog({ open, onClose, employee, onSuccess }: EmployeeDialogProps) {
  ...
  // Two separate forms — create needs password, update does not
    ...
    if (open) {
      ...
      if (employee) {
        updateForm.reset({
          ...
          role: employee.role as "CASHIER" | "MANAGER",
        });
      } else {
          ...
          <form onSubmit={updateForm.handleSubmit(onUpdateSubmit)} className="space-y-4 pt-1">
            ...
            <div className="space-y-1.5">
              ...
              {updateForm.formState.errors.role && (
                <p className="text-xs text-red-500">{updateForm.formState.errors.role.message}</p>
              )}
            </div>
...
}: { onClose: () => void; isSubmitting: boolean; isEditing: boolean }) {
  return (
